...
int main(int argc, char *argv[])
{
    ...
    if (argc == 3)
        snprintf(outfile, sizeof(outfile) - 1, "ips.lst", argv[1], argv[2]);
    else if (argc >= 4)
    {
        snprintf(outfile, sizeof(outfile) - 1, "ips.lst", argv[1], argv[3], argv[2]);
        bb = atoi(argv[3]);
        if ((bb < 0) || (bb > 255))
            ...
            {           
                ...
                else
                {
                    ...
                    for (x = 0; x < strlen(last); x++)
                        putchar('\b');
                    memset(&last, 0, sizeof(last));
                    snprintf(last, sizeof(last) - 1, "%s.%d.* [Victims: %d ] [Progress: %.1f%%]",
