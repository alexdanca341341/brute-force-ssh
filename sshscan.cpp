...
//       printf("handshake %s failed\n",parg->ip);
  ...
  {
    printf("\033[01;31mFounD -> \033[01;37m%s\033[01;31m:\033[01;37m%s \033[01;37m%s \033[01;31mPort:\033[01;37m %d \n",
     parg->user, parg->password ,parg->ip, parg->port );
    libssh2_session_disconnect(session,
			       "Normal Shutdown, Thank you for playing");
    libssh2_session_free(session);
    close(sockfd);
...
int checkSetting(int argc, char **argv, Setting *setting)
{
  ...
  else
  {
    ...
    strftime(time_buf, MAX_LEN_TIME_STR, "victims", tm_ptr);
    strncpy(setting->success_log_filename, time_buf,MAX_LEN_FILENAME);
    strncat(setting->success_log_filename,".log", MAX_LEN_FILENAME);
  }
